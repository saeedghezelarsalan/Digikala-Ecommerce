...
export default function HomePage() {
  ...
  const handleSliderChanges = (e: any, index: number) => {
  ...
  const handleSliderRemoves = (index: any) => {
    const list = [...slider];
    list.splice(index, 1);
    setSlider(list);
  };
  ...
  const handleBannerChanges = (e: any, index: number) => {
    const {name, value} = e.target;
    const list = [...banner];
    list[index][name] = value;
    setBanner(list);
  ...
  const handleBannerRemoves = (index: any) => {
    const list = [...banner];
    list.splice(index, 1);
    setBanner(list);
  };
    ...
    <div className='flex w-full xl:px-8 h-full min-h-screen bg-[#f14d60]'>
      ...
      <div className=' w-full xl:w-10/12 h-full flex flex-col bg-[#f14d60] py-4 px-3'>
        ...
        <form
          ...
          {slider.map((sliders: any, index: number) => (
          ...
          {banner.map((banners: any, index: number) => (
            <div key={index} className="flex flex-col gap-y-6">
              <div className="flex flex-col gap-y-2">
                <label>تصویر</label>
                <input
