...
export default function HomePage({ productss, brand, query, filterProduct, category, mainCategories, data, productsSubCategorySlug, productsCategorySlug }) {
  ...
  const filtersData = filterProducts && filterProducts.map(filter => filter).filter(filters => filters.subCategorySlug == query || filters.CategorySlug == query)
          ...
          <div className="w-[300px] border hidden flex-col lg:flex px-2 py-4">
            <div className="flex flex-col">
              ...
              <div className="cursor-pointer">
                <div onClick={openBrandHandler} className={`flex justify-between items-center cursor-pointer py-3 font-bold text-[15px] text-[#424750] ${openBrand ? "border-0" : "border border-x-0 border-t-0"} `}>
                  <p >برند</p>
...
export async function getServerSideProps({ params }) {
  ...
  let productss = data.map(a => a).filter((product) => product.subCategorySlug == query ? product.subCategorySlug == query : product.categorySlug == query ? product.categorySlug == query : null);

  const productsSubCategorySlug = data.map(a => a).filter((product) => product.subCategorySlug == query)

  const productsCategorySlug = data.map(a => a).filter((product) => product.CategorySlug == query)

  let brand = await axios.get("https://digikala-demo-data-q7eo.vercel.app/brand");
  brand = brand.data;

  let filterProduct = await axios.get("https://digikala-demo-data-q7eo.vercel.app/filterProduct");
